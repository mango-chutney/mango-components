...
const buttonThemeMixin = exports.buttonThemeMixin = (props, palette) => {
  if ([props.primary, props.secondary, props.tertiary, props.gray].filter(isTruthy => isTruthy).length > 1) {
    throw Error('...');
  }
...
const StyledButton = exports.StyledButton = _styledComponents2.default.a.withConfig({
  componentId: 's1db2ox4-0'
})(['background-color:', ';border:0;border-radius:0.25rem;color:', ';cursor:pointer;display:', ';font-weight:', ';margin-bottom:1rem;padding:0.5rem 1.25rem;text-decoration:none;text-align:center;'], _constants.palette.darkGray, _constants.palette.white, props => props.expanded ? 'block' : 'inline-block', _constants.fontWeights.semibold);
